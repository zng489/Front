...
exports.HTMLRoutingMiddlewarePlugin = exports.PrerenderPlugin = void 0;
 ...
 */
function HTMLRoutingMiddlewarePlugin({ fallback, } = {}) {
    let outDir;
    return {
        name: "serve-prerendered-html",
        configResolved(config) {
            outDir = node_path_1.default.resolve(config.root, config.build.outDir);
        },
        configurePreviewServer(server) {
            server.middlewares.use(async (req, _res, next) => {
                ...
                if (url.pathname != url.pathname.split(".").pop())
                    return next();
                const file = node_path_1.default.join(outDir, url.pathname.split(node_path_1.default.posix.sep).join(node_path_1.default.sep), "index.html");
                try {
...
exports.HTMLRoutingMiddlewarePlugin = HTMLRoutingMiddlewarePlugin;
